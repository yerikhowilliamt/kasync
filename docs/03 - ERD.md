...
**Status:** Draft v2.0
**Related:** [System_Design](./01%20-%20System_Design.md), [ADR](./02%20-%20ADR.md), [Schema Prisma](../prisma/schema.prisma), [Migration SQL](./database/migration.sql)
...
| Entity | Purpose |
|---|---|
| `User` | Authenticated user account with bcrypt-hashed password, hashed refresh token for session revocation, `photoUrl` (Cloudinary profile picture), and `tokenValidFrom` for immediate token revocation. |
| `Account` | A bank, cash, or e-wallet account belonging to the user. Scoped by `userId` with cascade delete. |
| `BankTransaction` | Atomic, immutable-in-practice record imported from a bank statement (has amount, type `INFLOW`/`OUTFLOW`, date, status, `dedupHash`). Scoped by `accountId`. |
| `Category` | Expense/income category scoped per user (`@@unique([userId, name])`). |
| `Branch` | Cost center / business branch scoped per user (`@@unique([userId, name])`). |
| `LedgerEntry` | A categorized manual record scoped by `userId`: category + branch + date + amount + type (`INFLOW`/`OUTFLOW`) + note. |
| `Allocation` | Junction between `BankTransaction` and `LedgerEntry`, carrying the portion of the amount allocated (with `status` and `revokedAt`). Optional `idempotencyKey` for retry safety. |

Full column-level detail lives in `prisma/schema.prisma` — this document explains the *design decisions*, not just the columns.

---

## 2. Relationships

- `User` 1 — N `Account`, `Category`, `Branch`, `LedgerEntry`: all domain entities are scoped to a single user (multi-tenancy via `userId` foreign key with `onDelete: Cascade`).
...
- `BankTransaction` 1 — N `Allocation`, `LedgerEntry` 1 — N `Allocation`: this is the many-to-many bridge — a transaction can be split into multiple allocations, and a ledger entry can receive allocations from multiple transactions (the "3 small deposits = 1 manual entry" case).
...
### 3.3 `status` stored directly on `BankTransaction`, not computed on every read
...
### 3.4 `Decimal(18, 2)` and `type` (`INFLOW`/`OUTFLOW`) for amounts
...
### 3.5 The allocation-sum constraint lives in a trigger with explicit row locking
...
### 3.6 No `category`/`branch` fields on `Allocation` itself
...
### 3.7 `idempotencyKey` on Allocation
An optional `idempotencyKey` field prevents duplicate allocation records from network retries or client bugs. When provided, the service checks for an existing allocation with that key before creating. The key is scoped per bank transaction via composite unique constraint `@@unique([bankTransactionId, idempotencyKey])` — different users may reuse the same key on different transactions, but within one transaction duplicates are rejected at the database level. See ADR-016.

### 3.8 Per-User Unique Constraints on Category and Branch
`Category.name` and `Branch.name` use `@@unique([userId, name])` instead of global `@unique`. This allows different users to create categories/branches with the same name while preventing duplicates within a single user's scope.
...
- `ledger_entries.user_id` — supports user-scoped queries for multi-tenancy.
...
- `accounts.user_id`, `categories.user_id`, `branches.user_id` — supports user-scoped CRUD queries.
